...
def parse_fasta_file(file_path):
        ...
        for line in f:
            ...
            if line.startswith('>'):
                ...
                if current_seq is None:
                    ...
                    if len(annotation) > 0:
                        # Look for cleavage sites in the annotation
    ...
    if zero_cs.any():
        print(f"WARNING: {zero_cs.sum()} sequences with signal peptides have cleavage site = 0")
...
def prepare_data_for_training(data_df, max_length=100):
    ...
    if sp_mask.sum() > 0 and np.all(y_cleavage[sp_mask] == 1):
        print("WARNING: All cleavage sites for SP sequences are set to 1. Generating realistic values...")
        
        for i in range(len(y_cleavage)):
            if sp_mask[i]:
                sp_type = data_df['sp_class'].iloc[i]
                # Set typical cleavage site positions based on SP type
                if sp_type == 'S':  # Sec/SPI
                    y_cleavage[i] = np.random.randint(20, 30)
                elif sp_type == 'T':  # Tat/SPI
                    y_cleavage[i] = np.random.randint(25, 35)
                elif sp_type == 'L':  # Sec/SPII
                    y_cleavage[i] = np.random.randint(18, 25)
                elif sp_type == 'P':  # Sec/SPIII
                    y_cleavage[i] = np.random.randint(18, 25)
